...
jest.mock('@tanstack/react-query', () => ({
  useMutation: () => ({
    mutateAsync: jest.fn().mockResolvedValue({ imageUrl: 'https://example.com/image.jpg' }),
  }),
}));
...
describe('ImageUpload', () => {
  ...
  describe('Rendering', () => {
    ...
    it('should render with custom label', () => {
      render(<ImageUpload value={undefined} onChange={mockOnChange} label="Custom Label" />);
  ...
  describe('Empty State', () => {
    it('should show upload prompt when no image', () => {
      render(<ImageUpload value={undefined} onChange={mockOnChange} />);
      expect(screen.getByText(/Drop image here, or click/i)).toBeInTheDocument();
    });

    it('should show max file size in empty state', () => {
      render(<ImageUpload value={undefined} onChange={mockOnChange} maxSize={5 * 1024 * 1024} />);
      expect(screen.getByText(/Max 5MB/i)).toBeInTheDocument();
    });

    it('should show Upload icon in empty state', () => {
      const { container } = render(<ImageUpload value={undefined} onChange={mockOnChange} />);
  ...
  describe('Label', () => {
    it('should render label when provided', () => {
      render(<ImageUpload value={undefined} onChange={mockOnChange} label="Store Logo" />);
      expect(screen.getByText('Store Logo')).toBeInTheDocument();
    });

    it('should not render label element when label is empty string', () => {
      const { container } = render(<ImageUpload value={undefined} onChange={mockOnChange} label="" />);
      const labels = container.querySelectorAll('label');
      expect(labels.length).toBe(0);
    });

    it('should apply correct styling to label', () => {
      render(<ImageUpload value={undefined} onChange={mockOnChange} label="Test" />);
  ...
  describe('Disabled State', () => {
    it('should disable file input when disabled prop is true', () => {
      render(<ImageUpload value={undefined} onChange={mockOnChange} disabled={true} />);
      const input = screen.getByTestId('file-input');
      expect(input).toBeDisabled();
    });

    it('should not disable file input when disabled prop is false', () => {
      render(<ImageUpload value={undefined} onChange={mockOnChange} disabled={false} />);
      const input = screen.getByTestId('file-input');
  ...
  describe('Image Preview', () => {
    ...
    it('should have alt text for uploaded image', () => {
      render(<ImageUpload value="https://example.com/path/to/image.jpg" onChange={mockOnChange} />);
      const image = screen.getByRole('img');
    ...
    it('should not render remove button when disabled', () => {
      render(<ImageUpload value="https://example.com/image.jpg" onChange={mockOnChange} disabled={true} />);
      const removeButton = screen.queryByRole('button', { name: /remove image/i });
  ...
  describe('Remove Image', () => {
    ...
    it('should render X icon in remove button', () => {
      const { container } = render(<ImageUpload value="https://example.com/image.jpg" onChange={mockOnChange} />);
  ...
  describe('Props', () => {
    it('should accept value prop', () => {
      render(<ImageUpload value="https://example.com/test.jpg" onChange={mockOnChange} />);
      const image = screen.getByRole('img');
    ...
    it('should accept maxSize prop', () => {
      ...
      render(<ImageUpload value={undefined} onChange={mockOnChange} maxSize={maxSize} />);
      expect(screen.getByText(/Max 5MB/i)).toBeInTheDocument();
    });
  ...
  describe('File Size Display', () => {
    it('should display file size in MB', () => {
      render(<ImageUpload value={undefined} onChange={mockOnChange} maxSize={10 * 1024 * 1024} />);
      expect(screen.getByText(/10MB/i)).toBeInTheDocument();
    });

    it('should round file size correctly', () => {
      render(<ImageUpload value={undefined} onChange={mockOnChange} maxSize={2.5 * 1024 * 1024} />);
